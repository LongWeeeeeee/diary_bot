...
BACKFILL_BUTTON = "Пропущенные дни 📆"
BACKFILL_WINDOW_DAYS = 30   # за какой период ищем пропуски для кнопок
BACKFILL_MAX_BUTTONS = 14   # сколько дат показываем кнопками (остальное — вводом даты)
...
class ClientState(StatesGroup):
    ...
    backfill_pick = State()      # выбор пропущенного дня
    backfill_tasks = State()     # отметка дел за выбранный день
